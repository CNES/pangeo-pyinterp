...
def test_spline():
    ...
    z = grid.spline(collections.OrderedDict(lon=x.flatten(), lat=y.flatten()))
    assert isinstance(z, np.ndarray)

    for fitting_model in [
            'linear', 'polynomial', 'c_spline', 'c_spline_periodic', 'akima',
            'akima_periodic', 'steffen'
    ]:
        other = grid.spline(collections.OrderedDict(lon=x.flatten(),
                                                    lat=y.flatten()),
                            fitting_model=fitting_model)
        assert (z - other).mean() != 0

    with pytest.raises(ValueError):
        grid.spline(collections.OrderedDict(lon=x.flatten(), lat=y.flatten()),
                    bounds_error=True)
    ...
    assert isinstance(grid, pyinterp.Grid2D)
    with pytest.raises(ValueError):
        pyinterp.spline(grid, x.flatten(), y.flatten(), fitting_model='_')
    with pytest.raises(ValueError):
        pyinterp.spline(grid, x.flatten(), y.flatten(), boundary='_')
    grid = pyinterp.Grid2D(x_axis.flip(inplace=False), y_axis, matrix.T)
    with pytest.raises(ValueError):
        pyinterp.spline(grid, x.flatten(), y.flatten())
