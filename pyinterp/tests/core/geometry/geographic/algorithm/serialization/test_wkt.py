...
"""Tests for WKT serialization and deserialization (geographic)."""
...
def test_point_roundtrip() -> None:
    ...
    assert abs(restored.lon - original.lon) < 1e-10
    assert abs(restored.lat - original.lat) < 1e-10
...
def test_wkt_high_precision() -> None:
    ...
    assert abs(restored.lon - point.lon) < 1e-5
    assert abs(restored.lat - point.lat) < 1e-5


def test_wkt_antimeridian() -> None:
    """Test WKT with coordinates crossing the antimeridian."""
    lon = np.array([175.0, 180.0, -175.0])
    lat = np.array([0.0, 0.0, 0.0])
    linestring = LineString(lon, lat)

    wkt = to_wkt(linestring)
    restored = from_wkt(wkt)
...
def test_wkt_polar_coordinates() -> None:
    """Test WKT with coordinates near poles."""
    point = Point(0.0, 89.5)
    wkt = to_wkt(point)
    restored = from_wkt(wkt)
...
def test_wkt_negative_longitude() -> None:
    """Test WKT with negative longitude."""
    point = Point(-123.45, 47.89)
    wkt = to_wkt(point)
    restored = from_wkt(wkt)
