...
axis = pyinterp.Axis(np.arange(-90, 90, 0.25, dtype=np.float64))
print(axis)
...
print(f"Is a circle? {axis.is_periodic}")

# %%
# A common operation is to find the index of the value closest to a given
# coordinate.
index = axis.find_index(np.array([1e-3]))
...
indices = axis.find_indexes(np.array([1e-3]))
...
# such an axis by setting the ``period`` parameter to ``360.0``.
axis = pyinterp.Axis(np.arange(0, 360, 1, dtype=np.float64), period=360.0)
print(axis)

# %%
# Now, the ``is_periodic`` property is ``True``.
print(f"Is a circle? {axis.is_periodic}")
...
index = axis.find_index(np.array([361.0]))
print(f"Index of 361 degrees: {index}")

index = axis.find_index(np.array([1.0]))
...
dates = np.datetime64("2020-01-01") + np.arange(0, 10**6, step=500).astype(
    "timedelta64[ms]"
)
...
axis.find_index(
    np.array(np.array(["2020-01-01T00:10:34.000"], dtype="datetime64[ms]")),
)

# %%
# You can pass any date unit to the axis.
axis.find_index(np.array(["2020-01-01"], dtype="datetime64[D]"))
# %%
# This object also makes it possible to manipulate timedeltas.
axis = pyinterp.TemporalAxis(dates - np.datetime64("2020-01-01"))
print(axis)
