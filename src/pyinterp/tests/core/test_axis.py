...
"""Unit tests for Axis: behavior, equality, flipping, and indexing."""
...
def test_axis_accessor() -> None:
    """Test Axis accessors and methods."""
  ...
  is_circle: false""")
    ...
    assert a.back() == 359
    assert np.all(
        a.find_index(np.arange(
            0,
            359,
            1,
            dtype=np.float64,
        ) + 0.01) == np.arange(0, 359, 1))
    indexes = a.find_indexes(np.arange(
        0,
        359,
        1,
        dtype=np.float64,
    ...
    with pytest.raises(ValueError):
        core.Axis([], is_circle=False)  # type: ignore[arg-type]

    with pytest.raises(ValueError):
        core.Axis([5, 2, 7], is_circle=False)  # type: ignore[arg-type]
...
def test_axis_pickle() -> None:
    ...
    a = core.Axis(np.linspace(0, 359, 360, dtype=np.float64), is_circle=False)
...
def test_find_index_circular() -> None:
    """Test find_index method for circular Axis."""
    a = core.Axis(np.arange(0, 360, 5, dtype=np.float64), is_circle=True)
    assert np.all(
        a.find_index(np.array([356, 358, -2, -4], dtype=np.float64)) ==
        np.array([71, 0, 0, 71]))
    a = core.Axis(np.arange(-180, 180, 5, dtype=np.float64), is_circle=True)
    assert np.all(
        a.find_index(np.array([176, 178, -182, -184], dtype=np.float64)) ==
        np.array([71, 0, 0, 71]))
