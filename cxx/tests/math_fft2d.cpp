...
inline auto fft_data() -> pyinterp::RowMajorMatrix<double> {
...
TEST(FFT2D, RoundtripDouble) {
  ...
  // Create the FFT plan
  math::FFT2D<double> fft(rows, cols);
  pyinterp::RowMajorComplexMatrix<double> c_data(rows, fft.c_cols());
...
TEST(FFT2D, RoundtripFloat) {
  ...
  pyinterp::RowMajorMatrix<float> data = fft_data().cast<float>();
  auto expected = data;  // The expected result is the original data

  // Create the DCT plan
  math::FFT2D<float> fft(rows, cols);
  pyinterp::RowMajorComplexMatrix<float> c_data(rows, fft.c_cols());
