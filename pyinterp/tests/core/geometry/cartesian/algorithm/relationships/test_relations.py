...
"""Tests for DE-9IM relation functions."""
...
def test_relation_different_points() -> None:
    ...
    assert isinstance(matrix, str)
    assert len(matrix) == 9
    # Disjoint points should have 'F' in specific positions
...
def test_relation_overlapping_polygons() -> None:
    ...
    assert isinstance(matrix, str)
    assert len(matrix) == 9
    # Overlapping polygons should have specific matrix
