...
(latitude and longitude) into a short string of digits and letters. This
string, also known as a geohash, delineates a rectangular area on a map, called
a cell. The longer the string, the smaller the cell and the more precise the
location.
...
from pyinterp import geohash, geometry
...
def _sort_colors(colors):
    ...
    by_hsv = sorted(
        (
            tuple(
                matplotlib.colors.rgb_to_hsv(matplotlib.colors.to_rgb(color))
            ),
            name,
        )
...
def _plot_box(ax, code, color, caption=True):
    ...
    box = geohash.GeoHash.from_string(code.decode()).bounding_box()
...
def plot_geohash_grid(
    precision, polygon=None, caption=True, color_list=None, inc=7
):
    ...
    if polygon is not None:
        box = (
            geometry.geographic.algorithms.envelope(polygon)
            if isinstance(polygon, geometry.geographic.Polygon)
            else polygon
        )
    ...
    ic = 0
    codes = geohash.bounding_boxes(polygon, precision=precision)
...
plot_geohash_grid(2, polygon=geohash.GeoHash.from_string("d").bounding_box())

# %%
# Bounds of the geohash ``dd`` with a precision of three characters.
plot_geohash_grid(3, polygon=geohash.GeoHash.from_string("dd").bounding_box())
# %%
# Bounds of the geohash ``dds`` with a precision of four characters.
plot_geohash_grid(4, polygon=geohash.GeoHash.from_string("dds").bounding_box())
...
code = geohash.GeoHash(-67.5, 22.5, precision=4)
...
# function.
codes = geohash.encode(lon, lat, precision=4)
print(codes)

# %%
# As you can see, the resulting codes are encoding as numpy byte arrays. This
# is to save memory and speed up the processing. This algorithm is very fast,
# which makes it possible to process a lot of data quickly.
#
# The following benchmark measures the time it takes to encode the coordinates
# into geohashes.
elapsed = (
    timeit.timeit(
        "geohash.encode(lon, lat, precision=12)",
        number=50,
        globals={"geohash": geohash, "lon": lon, "lat": lat},
    )
    / 50
)
...
# :py:func:`geohash.decode` function.
lon, lat = geohash.decode(codes)

# %%
# You can also use the :py:func:`geohash.transform` to transform
# geohashes from one precision to another.
codes = geohash.transform(codes, precision=1)
print(codes)

# %%
codes = geohash.transform(codes, precision=3)
print(codes)

# %%
# The :py:func:`geohash.bounding_boxes` function allows calculating the
...
# %%
coordinates = numpy.array(MEDITERRANEAN_SEA).T
polygon = geometry.geographic.Polygon(
    geometry.geographic.Ring(
        coordinates[0, :],
        coordinates[1, :],
    )
)
...
df = pandas.DataFrame(
    {
        ...
        "lat": lat,
        "measures": measures,
        "geohash": geohash.encode(lon, lat, precision=3),
    }
)
...
array = geohash.to_xarray(df.index.values.astype("S"), df.density.values)
array = array.where(array != 0, numpy.nan)

fig = matplotlib.pyplot.figure(figsize=(10, 5))
ax = fig.add_subplot(111, projection=cartopy.crs.PlateCarree())
array.plot(
    ax=ax, transform=cartopy.crs.PlateCarree(), cmap="viridis", robust=True
)
ax.coastlines()
