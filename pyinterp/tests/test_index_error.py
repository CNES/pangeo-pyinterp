...
"""Tests for index error handling in grid interpolations."""
import collections
import datetime
...
def test_index_error_3d() -> None:
    """Test index error handling in 3D grids."""
    ds = load_grid3d()
    ...
    with pytest.raises(ValueError, match=r'2020-07-02T15:00:00.000000000'):
        grid(
            collections.OrderedDict(longitude=x.ravel(),
                                    latitude=y.ravel(),
                                    time=t.ravel()),
    ...
    with pytest.raises(ValueError, match=r'1593702000000000000.000000'):
        grid(
            collections.OrderedDict(longitude=x.ravel(),
                                    latitude=y.ravel(),
...
def test_index_error_4d() -> None:
    """Test index error handling in 4D grids."""
    ds = load_grid4d()
    ...
    with pytest.raises(ValueError, match=r'2010-01-01T12:00:00.000000000'):
        grid.quadrivariate(
            collections.OrderedDict(longitude=x.ravel(),
                                    latitude=y.ravel(),
                                    level=z.ravel(),
